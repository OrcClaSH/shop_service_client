...
export const cartSlice = createSlice({
    ...
    reducers: {
        addItem: (state, action: PayloadAction<TCartProducts>) => {
            const findProduct = state.items.find(item => item.idProduct === action.payload.idProduct)
        ...
        changeCount: (state, action: PayloadAction<TChangeCountPayload>) => {
            ...
            if (findProduct && action.payload.change === '-' && findProduct.count > 0) findProduct.count -= 1;
            state.totalPrice = sumPrice(state.items);
        },
    }
});

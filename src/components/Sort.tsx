...
const Sort: React.FC = () => {
  ...
  const sortLabelClassName = classNames('sort__label', activePopup ? 'active' : '');

  const handleFilterClick = (sortMethod: string): void => {
    ...
    <div className="sort">
      <div className={sortLabelClassName} ref={sortLabelRef}>
        ...
        <span
          ...
          tabIndex={0}
          onKeyPress={(event) =>
            event.key === 'Enter' ? setActivePopup(!activePopup) : undefined
          }
        >
          {activeSortMethod}
        </span>
        ...
        <ul>
          {sortMethodList.map((sortMethod) => (
            <li
              ...
              tabIndex={0}
              onKeyPress={(event) =>
                event.key === 'Enter' ? handleFilterClick(sortMethod) : undefined
              }
            >
              {sortMethod}
            </li>
